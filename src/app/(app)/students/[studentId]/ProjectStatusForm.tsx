...
import { updateProjectStatus } from '@/features/students/actions';
...
const PROJECT_STATUSES = ['green', 'yellow', 'red'] as const;
...
export function ProjectStatusForm({
  ...
  const [selectedStatus, setSelectedStatus] = useState<TrafficLightStatus>(currentStatus);
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await updateProjectStatus(studentId, projectId, selectedStatus);

      if (!result.success) {
        setError(result.error ? t(result.error) : t('students.projectStatus.updateFailed'));
        return;
      }
    ...
    <form onSubmit={handleSubmit} className="space-y-3 rounded-lg border border-line p-3">
      <div className="space-y-1">
        <label htmlFor={selectId} className="block text-xs font-semibold text-ink-secondary">
          {t('students.projectStatus.selectLabel')}
        </label>
        <select
          id={selectId}
      ...
      {success ? (
        <p className="text-sm font-semibold text-status-positive" role="status">
          {t('students.projectStatus.updateSuccess')}
        </p>
      ) : null}
      ...
      >
        ...
        <span>{t('students.projectStatus.submitButton')}</span>
      </button>
    </form>
  );
}
