import Link from 'next/link';
import { ShieldAlert } from 'lucide-react';
import { getAdminGroupsData } from '@/features/groups/admin-queries';
import { GroupsWorkspace } from './GroupsWorkspace';
import { t } from '@/lib/i18n';

type AdminGroupsPageProps = {
  searchParams: Promise<{
    state?: string;
  }>;
};

function ForbiddenState() {
  return (
    <main className="min-h-screen bg-surface-sunken px-4 py-8 dark:bg-ink">
      <section className="mx-auto max-w-md rounded-2xl border border-line bg-white p-6 text-center shadow-xl dark:border-ink-secondary dark:bg-ink">
        <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-rose-600 text-white">
          <ShieldAlert className="h-6 w-6" />
        </div>
        <h1 className="text-xl font-bold text-ink dark:text-surface">
          {t('admin.accessGrants.forbiddenTitle')}
        </h1>
        <p className="mt-2 text-sm leading-6 text-ink-secondary dark:text-ink-muted">
          {t('admin.groups.errorForbidden')}
        </p>
        <Link
          href="/calendar"
          className="mt-5 inline-flex h-10 items-center justify-center rounded-xl bg-ink px-4 text-sm font-bold text-white transition-colors hover:bg-ink-secondary dark:bg-surface dark:text-ink dark:hover:bg-line"
        >
          {t('admin.accessGrants.backToDashboard')}
        </Link>
      </section>
    </main>
  );
}

export default async function AdminGroupsPage({ searchParams }: AdminGroupsPageProps) {
  const params = await searchParams;
  const data = await getAdminGroupsData(params.state);

  if (!data.isAuthorized) {
    return <ForbiddenState />;
  }

  return (
    <main className="p-4 md:p-6 lg:p-8">
      <div className="mx-auto flex max-w-6xl flex-col gap-6">
        <header className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm font-semibold text-accent-strong dark:text-accent-strong">
              {t('nav.admin')}
            </p>
            <h1 className="text-2xl font-bold tracking-tight text-ink dark:text-surface">
              {t('admin.groups.title')}
            </h1>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-ink-secondary dark:text-ink-muted">
              {t('admin.groups.description')}
            </p>
          </div>
        </header>

        {data.error ? (
          <div className="rounded-xl border border-rose-200 bg-rose-50 p-4 text-sm font-semibold text-rose-700 dark:border-rose-900/50 dark:bg-rose-950/20 dark:text-rose-300">
            {t(data.error)}
          </div>
        ) : null}

        <GroupsWorkspace
          groups={data.groups}
          mentorOptions={data.mentorOptions}
          schoolYearOptions={data.schoolYearOptions}
          stateFilter={data.state}
        />
      </div>
    </main>
  );
}
