  ...
  return createPortal(
    ...
    >
      ...
      >
        ...
        <p className="mb-4 text-xs font-semibold text-ink-muted dark:text-ink-muted">{group.name}</p>

        {mentorCountMismatch ? (
          <div
            className="mb-4 flex items-start gap-2 rounded-xl border border-status-caution-soft/50 bg-status-caution-soft dark:bg-status-caution-soft/20 dark:border-status-caution/50 px-3 py-2 text-xs font-semibold text-status-caution dark:text-status-caution"
            role="status"
          >
        ...
        <div className="space-y-2 mb-4">
          {activeMentors.length > 0 ? (
            activeMentors.map((mentor) => (
              <div
                key={mentor.id}
                className="flex items-center justify-between gap-2 rounded-xl border border-line dark:border-ink-secondary bg-surface dark:bg-ink/50 px-3 py-2"
              >
                <div className="min-w-0">
                  <p className="truncate text-sm font-semibold text-ink dark:text-surface-sunken">
                    {mentor.mentorName}
                  </p>
                  {mentor.isPrimary ? (
                    <span className="text-[10px] font-bold uppercase tracking-wider text-accent dark:text-accent-strong">
                ...
                <button
                  ...
                  className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-ink-muted hover:text-status-critical hover:bg-white dark:hover:bg-ink-secondary transition-colors disabled:opacity-50"
                >
            ...
            <p className="text-xs text-ink-muted dark:text-ink-secondary">{t('admin.groups.noMentorsAssigned')}</p>
          )}
        </div>

        <div className="flex items-end gap-2 border-t border-surface-sunken dark:border-ink-secondary pt-4">
          ...
          <button
            ...
            className="flex h-11 shrink-0 items-center gap-1.5 rounded-xl bg-accent hover:bg-accent-strong px-4 text-xs font-bold text-white transition-colors disabled:opacity-50"
          >
