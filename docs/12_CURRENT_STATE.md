...
## Student card tabs (Staff App Redesign v1 Feedback Fixes)

**Supersedes the single-page layout described in the Student card status section above.** `/students/[studentId]` is now organized into 3 tabs (`src/app/(app)/students/[studentId]/StudentCardTabs.tsx`) instead of one long scrolling page: "פרטים" (overview — contact info, project, emotional status, comments/messages, message composer), "מטרות" (goals, read-only list), and "עדכון פרטים" (every edit form — project status, emotional status, goal management — consolidated in one place). The identity header (avatar, name, group, track/layer, mentors, follow, photo upload) stays above the tabs, unchanged. No schema, permission, or query changes — same `getStudentCard` data, same authorization flags, IA-only restructure. Full detail: `docs/30_STAFF_APP_REDESIGN_V1_FEEDBACK_FIXES.md` item H.
...
- **Multiple Views**: Upgraded the interface to support **List**, **Day**, **Week**, **Month**, and **Year (Gantt)** views.
...
## Google Calendar outbound sync v1 and browser verification status

Outbound-only Google Calendar service-account mirror sync exists at `/admin/import-export` (batch preview/run) and `/admin/calendar` (single-event "Sync Now"). Full architecture and mapping rules: `docs/25_GOOGLE_CALENDAR_OUTBOUND_SYNC.md`. Full browser verification report: `docs/26_GOOGLE_CALENDAR_SYNC_BROWSER_VERIFICATION.md`.

Status:

- **Live-verified against a dedicated test Google Calendar** (`לוח שנה החממה`) and test service-account credentials configured locally by the human operator, never the real production school calendar. No real credentials or `.env.local` were committed at any point.
- Verified live and passing: disabled/unconfigured regression, preview sync (confirmed no Google mutations occur), insert sync, update sync (in-place, no duplication — the same `google_calendar_event_id` before and after), single-event sync via the sidebar "Sync Now" button (only rendered for unsynced events), all-day mapping, multi-day mapping, remote-deletion recovery, and local-to-Google deletion.
- Audit trail confirmed correct and credential-free for all sync actions: `calendar_google_sync.previewed`, `.started`, `.completed`, `calendar_google_event.inserted`, `.updated`, `.deleted`, `.delete_failed`, and the newly added `.recreated`.
- Code review confirmed the Google-deletion failure policy matches documentation: local deletion always proceeds even if the remote Google delete fails (logged as `calendar_google_event.delete_failed`), and a `404` on delete is silently ignored.
- Permission checks (`checkAdminAuth()`, backed by `current_user_is_manager_or_super_admin()`) are shared across all three sync server actions; verified by code review only, since no second real Google-authenticated test account was available this session.
- No credential leakage was found anywhere: `src/lib/google/calendar-client.ts` is guarded by `import 'server-only'`; a full-text search of the built `.next/static` client bundles for private-key markers found zero matches; the sync UI and all sync action return types carry only booleans, counts, error strings, and event IDs.
...
3. **Calendar Recurrence Rules v1 and drag-and-drop editing**: optional later.
4. **Real-data Student/Staff Import**: paused until next school year rollout planning.
...
## Latest Admin Calendar Operations v1 results

Implemented administrative planning views, import, and export tools driven by immediate school Gantt workflows:
- **Annual Gantt / Year Planning view**: Enabled `view=year` at `/admin/calendar` displaying events as horizontal timeline bars mapped dynamically to month columns across the selected school year. Swapping between school years loads overlapping calendar events correctly.
- **Calendar Event CSV Export**: Implemented GET route at `/api/admin/calendar/export` exporting calendar events to RFC-4180 double-quoted CSV format. Logs `calendar_events.exported` audit events.
- **Calendar Event CSV Import**: Implemented validation engine and preview UI at `/admin/import-export`. Checked title, dates order, visibility, and matching group name/IDs, batch-creating events with server-side pre-write validation and best-effort batch creation, auditing `calendar_event.created`.
- **Navigation Activation**: Wired Sidebar navigation for Import/Export, making the page accessible to managers and super admins.

Scope boundaries preserved:
- No real student or calendar data was used or imported.
- No secrets or credentials were committed.
- Google Calendar API calls and outbound sync were deferred at this point.
- Supabase RLS is fully maintained.

## Latest Google Calendar Outbound Sync v1 results

Implemented safe, admin-controlled, outbound-only Google Calendar sync from the app's `calendar_events` table to one configured institutional Google Calendar:
- **Server-Side Auth Model**: Implemented JWT authorization via Google Service Account using `googleapis` dependency, configured entirely via server-only env variables. No individual admin OAuth permission flow required.
- **Event Mapping**: Converts local database rows to Google Calendar event resources with proper timezone handling (`Asia/Jerusalem`), exclusive date bounds for all-day events, and private extended properties tracking (`chamama_event_id` and `chamama_source = 'staff_app'`).
- **Sync Actions**: Built server actions for sync preview (`previewGoogleCalendarSyncAction`), run sync (`runGoogleCalendarSyncAction`), and single event sync (`syncSingleCalendarEventAction`). Handles remote deletions gracefully by re-inserting events. Halts immediately on auth errors, but continues on event-specific errors.
- **UI Panels**: Integrated the sync mirror dashboard card on `/admin/import-export` (showing preview counts, run results, warnings, and errors) and compact "Sync Now" button inside the Edit Sidebar drawer on `/admin/calendar`.
- **Resilient Deletions**: Remote Google deletion is triggered on local event deletion. If the Google API throws an error (or returns 404), the local database delete still proceeds to prevent administrator lockouts.
- **Auditing**: Added full audit logs for preview, start, completion, insertion, update, and deletion of synced events.

Scope boundaries preserved:
- No real credentials or `.env.local` changes were committed.
- No real calendar or student data was used or committed.
- Supabase RLS policies are fully maintained.
- Outbound only; no inbound sync or conflict resolution.
...
- **Follow-up**: this pass's changes were committed by Ronen shortly after (`git log`: `7642d5a design change`, `c34e483 design change fix`), and real usage on the deployed app surfaced further issues — see "Latest Staff App Redesign v1 Feedback Fixes results" below.

## Latest Staff App Redesign v1 Feedback Fixes results

Two follow-up passes after real usage of the deployed redesign. Full report: `docs/30_STAFF_APP_REDESIGN_V1_FEEDBACK_FIXES.md`; parallel handoff: `docs/parallel/GPT_STAFF_APP_REDESIGN_V1_FEEDBACK_FIXES_HANDOFF.md`.

- **Round 1 (8 items of direct feedback)**: bottom-nav icons rewritten to match the mockup's exact SVG path data; Settings now shows a general admin entry point for any manager (not just super-admins, who were previously the only role with any UI path into `/admin/*` despite managers having real access); push-notification config investigated and confirmed already fully working (no code change — if broken in production, most likely a missing `NEXT_PUBLIC_VAPID_PUBLIC_KEY` env var on the deploy platform); Settings logo replaced with version text; school emblem added to every tab header (and a second broken-SVG-asset bug found and fixed along the way, same class as the two logo SVGs from the original pass); "תלמיד/תלמידים" replaced with "חניך/חניכים" throughout `he.json` (matching wording the mockup's own source already used); student card restructured into 3 tabs (see "Student card tabs" section above).
- **Round 2 (production session-loss root cause — later corrected)**: Round 1's fix for the reported session-loss bug (sequencing two calls in `getMessagesFeed()`) turned out to address only one instance of a systemic issue, not the root cause. **This round's diagnosis was itself incorrect and was corrected in Round 3.** It claimed memoizing `createClient()` via React's `cache()` closed cross-request refresh races; `cache()` only dedupes calls within a single Server Component render pass / Server Action invocation / Route Handler invocation and cannot coordinate separate requests (middleware vs. a page render, two tabs, two unrelated concurrent requests). See Round 3 below for the corrected root cause and fix. `AdminShell.tsx`'s `prefetch={false}` fix from this round (reducing a burst of ~7 simultaneous auth-middleware requests from the desktop sidebar's link prefetching) was real and is retained.
- **Round 3 (corrected root cause and fix)**: redone from the actual auth code and the installed `@supabase/ssr`/`@supabase/supabase-js` library source rather than inference, after Ronen supplied a detailed technical correction. Found and fixed four confirmed bugs in `src/proxy.ts`: (1) redirects dropped cookie attributes (`path`/`httpOnly`/`secure`/`sameSite`/`maxAge`) by forwarding only name+value instead of the full cookie object — fixed; (2) the CDN no-cache headers (`Cache-Control: private, no-cache, no-store...`) that `@supabase/ssr`'s `setAll(cookiesToSet, headers)` passes specifically to stop a response carrying fresh auth cookies from being cached by an edge/CDN layer were silently dropped because `setAll` only declared one parameter — this app is on Vercel, which has exactly such a layer that local dev doesn't, making this the leading candidate for why the symptom was production-only and correlated with longer sessions; (3) an RPC error from `current_user_is_active_staff` was indistinguishable from a genuine "not active staff" result and forced an `/access-pending` redirect over a transient DB/network failure — fixed to pass the request through untouched instead (same fix applied to the equivalent path in `src/lib/auth/session.ts`); (4) switched from `getUser()` to `getClaims()`, which verifies locally via WebCrypto/JWKS (this project's tokens use asymmetric ES256 signing, confirmed by decoding a real token) instead of a network round-trip on every request. `cache()` in `src/lib/supabase/server.ts` was corrected to a within-invocation-only dedup optimization, with an explicit comment stating it does not provide cross-request coordination. `BottomNav.tsx`'s unread-count fetch (a separate Server Action firing on nearly every navigation) was deferred 400ms as an additional practical mitigation. Full report: `docs/30_STAFF_APP_REDESIGN_V1_FEEDBACK_FIXES.md`.
- **Local expiry test result — framed correctly**: per explicit instruction not to declare success from a fresh-token walkthrough alone, ran a deterministic test with `supabase/config.toml`'s `jwt_expiry` temporarily set to 15 seconds (restored afterward, confirmed via `git diff --stat` showing no residual change). A single authenticated request past-expiry correctly refreshes and succeeds. Multiple authenticated requests landing close together in time occasionally produced `refresh_token_already_used`. **This was an intentionally extreme stress test, not a realistic scenario** — Supabase recommends not running production expiry below five minutes, and at 15 seconds essentially every request (including the app's own incidental background ones) attempts a refresh, which manufactures far more collision opportunities than production's ~1-hour cycle would. A residual risk was observed only under this intentionally unrealistic 15-second stress test; **its relevance to production is unknown** and this result does not by itself prove anything about production behavior. It does not justify new coordination infrastructure — see "Next recommended tasks" below for the actual diagnostic path.
- **Round 4 (pre-deploy closeout)**: removed React `cache()` from `src/lib/supabase/server.ts` entirely rather than retaining it as a "dedup optimization" — a generic factory shared by Server Components, Server Actions, and Route Handlers shouldn't depend on a React-render-scoped primitive at all. `createClient()` is now a plain uncached `async function`. The one real duplicate-client case (`getMessagesFeed()`) now explicitly creates one client and passes it into `getAnnouncements()`/`getNotifications()` (both accept an optional shared-client parameter). The `[proxy-auth]` diagnostic logging is now gated behind a server-only `AUTH_DIAGNOSTICS_ENABLED` env var (unset = silent; documented in `.env.example`) instead of always-on. `BottomNav.tsx`'s deferred-fetch safety (timer cancellation, no post-unmount state updates, no firing on admin routes or unrelated navigations) was re-verified line-by-line, and its code comment reworded to state it's a mitigation, not a claim that it resolves refresh races. Full report: `docs/30_STAFF_APP_REDESIGN_V1_FEEDBACK_FIXES.md`.
- **Round 5 (production logout fixes)**: resolved the production session-loss redirect bug. (1) Disabled prefetch on Notifications and Sign Out Links on Settings page (`prefetch={false}`). (2) Stopped BottomNav's initial-mount background unread count fetch (removed `prevPath === null` as an automatic reason to fetch) to prevent background auth requests on page loads. (3) Classified background requests (RSC/prefetch/Server Action/Accept non-HTML) in `src/proxy.ts` and allowed them to pass through safely without a redirect on claims failures, returning a controlled non-redirect response and logging as `background_claims_error`. (4) Reduced page-level Auth-server calls by replacing `getUser()` with cryptographic claims lookup `getClaims()` and using `claimsData?.claims?.sub` in profiles/notifications queries, admin-queries, and notification actions. Full report: `docs/30_STAFF_APP_REDESIGN_V1_FEEDBACK_FIXES.md`.
- **Verification**: `tsc`, lint, Hebrew scanner, and `next build` all clean after every change across all five rounds. Live-verified fresh-session walkthroughs (Settings → Notifications → Admin → Calendar) with no regressions; anonymous users still redirect to `/login`; inactive profiles still land on `/access-pending`, not `/login`.
- Nothing committed by this session either — same working-tree-only convention as before.

## Next recommended tasks

1. **Commit, deploy, and monitor the corrected Proxy/cookie flow through a real production token-refresh window.** This is the immediate next step, not new infrastructure. Set `AUTH_DIAGNOSTICS_ENABLED=true` for the monitoring window and check Vercel's logs for `/settings`, `/notifications`, `/admin/groups`, `/calendar`, `/messages`.
2. **If `refresh_token_already_used` appears in production logs**, the diagnostic path is: (a) correlate requests by the logged request ID; (b) confirm the successful refresh response actually sent `Set-Cookie`; (c) confirm the browser used the new cookie on the following request; (d) identify any stale/delayed request still using the previous cookie; (e) confirm the requests hit the same production domain and deployment. Only consider coordination infrastructure (e.g. a distributed lock) after those facts are proven — not preemptively. Never key logs or any future locking mechanism using raw refresh tokens.
3. **Human visual pass on Staff App Redesign v1**: confirm the running app matches the mockup pixel-for-pixel, since most verification across all redesign passes has been DOM/computed-style/behavioral rather than a full visual side-by-side.
4. **Admin Students, Users, and Settings Usability v1**: activate the remaining disabled `AdminShell` placeholders (Students, Users, Settings) as real, usable admin surfaces.
5. **Staff/Student Import Apply Gate v1**: optional later. Design and wire the DB apply gate in the browser UI, to be enabled only after explicit rollout approval.
6. **Google Calendar Sync Delete/Conflict Hardening v1**: optional later. Design and implement remote conflict resolution.
