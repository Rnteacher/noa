...
- **Verification**: `tsc`, lint, Hebrew scanner, and `next build` all clean after every change across all four rounds. Live-verified fresh-session walkthroughs (Settings → Notifications → Admin → Calendar) with no regressions; anonymous users still redirect to `/login`; inactive profiles still land on `/access-pending`, not `/login`.
