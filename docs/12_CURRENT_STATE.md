...
- Deferred: outbound Google Calendar Sync API integration, recurrence rule mutation support, and full visual drag-and-drop slots editing.
...
- No secrets or credentials were committed.
- Google Calendar API calls and outbound sync remain deferred as a documented next task.
- Supabase RLS is fully maintained.

Updated recommended next task:

1. **Google Calendar Outbound Sync v1**: Implement the background sync client, linking database updates on `public.calendar_events` directly to Google Calendar Event resources via Google Calendar API, tracking status via `google_calendar_event_id`.
