...
5. **Remaining manual verification**: An actual authenticated-browser smoke test was completed (see "Latest authenticated browser smoke test results" below), but a few items still need manual coverage: uploading a real student photo file (the automated browser tool could not upload a locally-generated test file due to session file-sharing sandboxing), wrong-domain Google account rejection (no non-institutional test account was available), and a genuine two-account cross-user notification click-test (verified instead via direct database inspection of a pre-existing seeded follower).
