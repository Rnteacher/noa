...
- **Student photo upload optimization v2 is implemented and browser/manual-verified.** Authorized group mentors, managers, and super admins can upload or replace a student's profile photo. Before uploading, the image is optimized browser-side via canvas center-cropping, resized to a square 512x512 pixels, and converted to WebP (`image/webp`) at quality 0.82. The server action enforces a strict 1MB size limit (down from 5MB) and accepts only the optimized `image/webp` type. Files are stored in a private bucket (`student-photos`) under the path `students/${studentId}/profile.webp` and resolved via secure signed URLs. Updates write secure audit logs (`student_photo.updated`) via the secure definer RPC helper `update_student_photo_path`, which also validates that the new path is scoped to `students/{target_student_id}/`. Advanced interactive cropping, image moderation, and bulk photo imports remain deferred.
  ...
  - **Finding (not fixed, documented precisely)**: unlike the doc wording implied, direct `students.photo_url` updates are not "impossible outside the RPC" for *privileged* roles — a manager/super_admin session (which already holds broader `UPDATE` rights on the `students` table for other student-editing flows) **can** issue a raw `UPDATE students SET photo_url = 'anything'` that bypasses the RPC's path-format check entirely (confirmed live: a privileged direct update to an arbitrary non-`students/{id}/`-prefixed string succeeded). This is not exploitable by unauthorized users — it requires already holding a valid privileged session — but it is a real defense-in-depth gap versus the RPC's validation. No fix was applied in this pass (would require a schema change — e.g. a trigger or column-scoped policy — outside this task's "no schema changes unless a real blocker" scope); flagged as a follow-up hardening item below.
...
- **Follow and notifications**: following/unfollowing Alice Smith worked (confirmed via the UI toggle state and the database `followed_students` row). A genuine two-account, real-time cross-user notification click-test was not performed (only one authenticated Google account was available in this session); instead, the full notification pipeline was rigorously verified by inspecting the database directly: every mutation performed during this test (message, project status, emotional status, goal actions) correctly generated a notification for a pre-existing seeded follower (`mentor.one`, not the test actor), with generic, privacy-safe text (no message body, no emotional color/value, no goal title/description), and the acting user never received a notification for their own actions.
...
- **Student photo upload**: fully browser/manual-verified in a later dedicated pass (the user personally selected two real images). See "Student photo upload optimization v2" earlier in this document for the complete real-image test results, storage/audit verification, and the one defense-in-depth finding (direct-update bypass by privileged roles, not fixed, documented). No longer a leftover.
- **Wrong-domain Google rejection**: Google OAuth callback allowed-domain checks and sign-out logic were fully code-reviewed and verified, confirming domain mismatches route to `/access-denied` and terminate the session. A live sign-in with an external wrong-domain Google account remains manual-only.
- **Two-account notification**: The follower-notification database trigger pipeline (excluding the actor, filtering out muted follows), RLS read/update scopes, and unread badge count updates are verified. Transaction-rollback database SQL probes from the previous task confirmed all pipeline steps. A live multi-user browser verification remains manual-only.
...
3. **Manual verification leftovers**: Run live browser verification for OAuth wrong-domain Google account rejection and cross-user real-time notification badge click-testing. Student photo upload is no longer a leftover (fully browser/manual-verified with real images — see "Student photo upload optimization v2" above). These remaining items are code-reviewed and database-verified, but live browser-level paths remain manual-only.
